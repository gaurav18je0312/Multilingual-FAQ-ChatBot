...
with open(file_path, 'r') as f:
    tmp = json.load(f)
    prompt_json = tmp["prompt"]
    document_json = tmp["document"]
    question_json = tmp["question"]
...
def format_system_message(lang_id) -> dict:
    ...
    return {"role": "system", "content": prompt_json["en"]}
...
def format_user_message(lang_id, question, document_list=[], max_tokens=1280):
    ...
    user_prompt = f"{question_json[lang_id]}: {question}\n\n{document_json[lang_id]}:\n\n{doc_string}"
...
def search_documents(question: str, limit: int = 5) -> list:
    ...
    results = mq.index("farmer-scheme").search(question)
    lst = []
    print(results)
...
def create_message_payload(user_message, system_message, messages=[], max_tokens=3000):
    ...
    message_history.insert(0,user_message)
